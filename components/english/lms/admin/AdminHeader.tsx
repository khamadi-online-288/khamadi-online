...
export default function AdminHeader({ title }: { title?: string }) {
  ...
  return (
    <header style={{ height: 62, background: '#fff', borderBottom: '1px solid rgba(27,58,107,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', position: 'sticky', top: 0, zIndex: 50, fontFamily: 'Montserrat, sans-serif' }}>
      <div style={{ fontSize: 17, fontWeight: 900, color: '#0f172a' }}>{title || t.admin.dashboard_title}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontSize: 12 }}>
            <div style={{ fontWeight: 800, color: '#0f172a' }}>{name || t.common.administrator}</div>
            <div style={{ fontSize: 10, color: '#94a3b8' }}>{t.common.administrator}</div>
          </div>
        </div>
      </div>
    </header>
