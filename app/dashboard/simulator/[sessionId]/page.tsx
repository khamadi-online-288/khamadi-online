...
export default function SimulatorSessionPage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        if (!user) {
          setProfile1('Шет тілі')
          setProfile2('Дүниежүзі тарихы')
        ...
        const { data, error } = await supabase
          .from('profiles')
          .select('profile_subject_1, profile_subject_2, full_name')
          .eq('id', user.id)
          .single()
  ...
  useEffect(() => {
    ...
    async function generateQuestions() {
      try {
        ...
        const safeProfile1 = profile1 || 'Шет тілі'
        const safeProfile2 = profile2 || 'Дүниежүзі тарихы'

        const subjects = [
          { name: 'Қазақстан тарихы', id: 1, count: 20 },
          { name: 'Математикалық сауаттылық', id: 2, count: 10 },
          { name: 'Оқу сауаттылығы', id: 3, count: 10 },
          { name: safeProfile1, id: 4, count: 40 },
          { name: safeProfile2, id: 5, count: 40 },
        ]
        ...
        for (let i = 0; i < subjects.length; i++) {
          ...
          if (!res.ok) {
            throw new Error(data?.error || `${subject.name} сұрақтары жасалмады`)
          }

          if (!data?.questions || !Array.isArray(data.questions)) {
            throw new Error(`${subject.name} үшін сұрақтар келмеді`)
          }

          data.questions.forEach((q: any) => {
            allQuestions.push({
              id: q.id || `ai_${subject.id}_${globalNumber}`,
              subjectId: subject.id,
        ...
        if (!allQuestions.length) {
          throw new Error('Сұрақтар мүлде жасалмады')
        }
        ...
        setLoadingError(e?.message || 'Сұрақ генерациясында қате шықты')
      } finally {
    ...
    generateQuestions()
  }, [profileLoaded, profile1, profile2, sessionId])
  ...
  const handleNext = () =>
    setCurrentIndex((prev) => Math.min(currentSubjectQuestions.length - 1, prev + 1))
    ...
    return (
      <div style={styles.loadingPage}>
        <div style={styles.loadingCard}>
          ...
          <div style={styles.loadingTitle}>AI сұрақтар генерациялауда...</div>
          <div style={styles.loadingSubject}>{loadingSubject}</div>
          <div style={styles.progressBar}>
            <div style={{ ...styles.progressFill, width: `${loadingProgress}%` }} />
          </div>
          <div style={styles.loadingPercent}>{loadingProgress}% дайын</div>
    ...
    return (
      <div style={styles.loadingPage}>
        <div style={styles.errorCard}>
          <div style={styles.errorTitle}>Генерация қателігі</div>
          <div style={styles.errorText}>{loadingError}</div>
  ...
  if (!currentQuestion) {
    return (
      <div style={styles.loadingPage}>
        <div style={styles.errorCard}>
          <div style={styles.errorTitle}>Сұрақтар табылмады</div>
        </div>
      </div>
    )
  }
  ...
  const options = [
    ...
    { key: 'G', text: currentQuestion.optionG },
      ...
      <div style={styles.topBar}>
        <div style={styles.topWrap}>
          <div style={styles.leftHeader}>
            <button onClick={() => router.push('/dashboard/simulator')} style={styles.backBtn}>
              ← Артқа
            </button>
            <div>
          ...
          <div style={styles.rightHeader}>
            <div
              style={{
                ...
                border: timeLeft < 300 ? '1px solid #FCA5A5' : '1px solid #BAE6FD',
              }}
            >
              {formatTime(timeLeft)}
            </div>

            <button style={styles.secondaryBtn} onClick={() => setShowCalculator(true)}>
              Калькулятор
            </button>

            <button style={styles.secondaryBtn} onClick={() => setShowPeriodicTable(true)}>
              Менделеев
            </button>
            ...
            return (
              ...
              >
                ...
                <span style={styles.tabMeta}>
                  {subjectAnsweredCount(tab.id)} / {tab.count}
                </span>
              </button>
            )
          })}
        ...
        <div style={styles.layout}>
          <div style={styles.mainCard}>
            <div style={styles.metaRow}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                {currentQuestion.isMultiple && (
                  <div style={styles.multipleBadge}>2 дұрыс жауап таңда</div>
                )}
            ...
            <div style={styles.questionCount}>
              Сұрақ {currentQuestion.number} / {questions.length}
            </div>
            ...
            <div style={styles.bottomActions}>
              <div style={styles.bottomLeft}>
                <button
                  ...
                  style={{
                    ...styles.navBtn,
                    background: flagged[currentQuestion.id] ? '#FEF3C7' : '#FFFFFF',
                  }}
                >
                ...
                <button
                  ...
                  style={{ ...styles.navBtn, opacity: currentIndex === 0 ? 0.45 : 1 }}
                >
                  ← Артқа
                </button>
                ...
                <button
                  ...
                  style={{
                    ...
                    opacity: currentIndex === currentSubjectQuestions.length - 1 ? 0.45 : 1,
                  }}
                >
                  Келесі →
                </button>
          ...
          <div style={styles.sidePanel}>
            <div style={styles.sideCard}>
              ...
              <div style={styles.sideText}>
                Жалпы жауап: <strong>{totalAnswered}</strong> / {questions.length}
                <br />
                Ағымдағы пән: <strong>{currentQuestion.subjectName}</strong>
                <br />
        ...
        <div style={styles.overlay} onClick={() => setShowCalculator(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              ...
              <button style={styles.secondaryBtn} onClick={() => setShowCalculator(false)}>
                Жабу
              </button>
            </div>
            ...
            <div style={styles.calcGrid}>
              {['7', '8', '9', '÷', '4', '5', '6', '×', '1', '2', '3', '-', '0', '.', '%', '+'].map(
                (item) => (
                  <button key={item} style={styles.calcBtn} onClick={() => calcInput(item)}>
                    {item}
                  </button>
                )
              )}

              <button style={styles.calcBtn} onClick={calcClear}>
                C
              </button>
              <button style={styles.calcBtn} onClick={calcBackspace}>
                ⌫
              </button>
              <button style={styles.calcBtn} onClick={() => calcInput('(')}>
                (
              </button>
              <button style={styles.calcBtn} onClick={() => calcInput(')')}>
                )
              ...
              <button style={styles.calcEqualBtn} onClick={calcEquals}>
                =
              </button>
            </div>
          </div>
        ...
        <div style={styles.overlay} onClick={() => setShowPeriodicTable(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              ...
              <button style={styles.secondaryBtn} onClick={() => setShowPeriodicTable(false)}>
                Жабу
              </button>
            </div>
