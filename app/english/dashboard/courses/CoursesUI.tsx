...
interface Props {
  generalCourses: CourseWithProgress[]
  trackEnriched: CourseWithProgress | null
}

export default function CoursesUI({ generalCourses, trackEnriched }: Props) {
  const { t } = useLanguage()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      <section>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 900, color: '#1B3A6B', letterSpacing: '-0.01em' }}>General English</span>
      ...
      {trackEnriched && (
        <section>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 900, color: '#1B3A6B', letterSpacing: '-0.01em' }}>{t.courses.pro_track}</span>
...
function CourseCard({ course, isMyTrack = false }: { course: CourseWithProgress; isMyTrack?: boolean }) {
  ...
  return (
    <Link
      ...
      style={{ textDecoration: 'none', display: 'flex', outline: isMyTrack ? '2px solid #C9933B' : undefined, flexDirection: 'column', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 20px rgba(27,58,107,0.08)' }}
    >
      <div style={{ background: cardStyle.bg, height: 140, borderRadius: '16px 16px 0 0', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '20px 24px' }}>
        ...
        {course.pct === 100 && !isESP && (
          <div style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(16,185,129,0.25)', border: '1px solid rgba(16,185,129,0.4)', borderRadius: 99, padding: '3px 10px', fontSize: 10, fontWeight: 800, color: '#6ee7b7' }}>
            {t.courses.completed_badge}
          </div>
        )}

        {!isESP && levelKey && (
          <>
            <div style={{ fontFamily: 'Georgia, serif', fontSize: 48, fontWeight: 700, color: '#ffffff', lineHeight: 1 }}>{levelKey}</div>
            <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.6)', letterSpacing: '2px', marginTop: 4 }}>{label}</div>
        ...
        {isESP && (
          <>
            <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.9)', letterSpacing: '2px', textTransform: 'uppercase' as const }}>{course.level ?? 'B1 – C1'}</div>
            <div style={{ width: 36, height: 2, background: '#C9933B', borderRadius: 1, marginTop: 8 }} />
          </>
        )}
      </div>

      <div style={{ background: '#fff', borderRadius: '0 0 16px 16px', border: '1px solid rgba(27,58,107,0.08)', borderTop: 'none', padding: '16px 24px 20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#1B3A6B', marginBottom: 6, lineHeight: 1.3 }}>{course.title}</div>
        <div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5, marginBottom: 16, minHeight: 36, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {course.description ?? t.courses.start_hint}
        </div>
        <div style={{ marginTop: 'auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: '#94a3b8' }}>{course.done} / {course.total} {t.courses.lessons}</span>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#1B3A6B' }}>{course.pct}%</span>
          </div>
