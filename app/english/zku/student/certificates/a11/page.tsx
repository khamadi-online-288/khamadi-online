...
export default function A11CertificatePage() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F8FAFC' }}>
        <div style={{ width: 40, height: 40, border: `3px solid ${GR}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      </div>
    )
  }

  const certId = `WKU-A11-${Date.now().toString(36).toUpperCase().slice(-8)}`
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#F0F4F8', padding: '40px 24px 60px', display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
        <div className="cert-page" style={{
          ...
          {/* Outer green border */}
          <div style={{ position: 'absolute', inset: 14, border: `2px solid ${GR}`, borderRadius: 2, pointerEvents: 'none', zIndex: 1 }} />
          <div style={{ position: 'absolute', inset: 18, border: `1px solid rgba(22,163,74,0.3)`, borderRadius: 2, pointerEvents: 'none', zIndex: 1 }} />
          ...
          {[{t:8,l:8},{t:8,r:8},{b:8,l:8},{b:8,r:8}].map((pos, i) => (
            <div key={i} style={{
              ...
              borderTop: pos.t !== undefined ? `3px solid ${GR}` : 'none',
              borderBottom: pos.b !== undefined ? `3px solid ${GR}` : 'none',
              borderLeft: pos.l !== undefined ? `3px solid ${GR}` : 'none',
              borderRight: pos.r !== undefined ? `3px solid ${GR}` : 'none',
            }} />
          ))}
          ...
          <div style={{
            ...
            <div style={{ position: 'absolute', right: -40, top: -40, width: 160, height: 160, borderRadius: '50%', background: 'rgba(201,147,59,0.1)' }} />
            ...
            <div style={{ color: '#BBF7D0', fontSize: 28, fontWeight: 900, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              A1.1 English — Elementary Level
            </div>
          </div>
          ...
          <div style={{ padding: '44px 60px' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 36 }}>
              ...
              <div style={{ fontSize: 34, fontWeight: 900, color: N, borderBottom: `2px solid ${GR}`, paddingBottom: 10, display: 'inline-block', minWidth: 300, letterSpacing: '-0.5px' }}>
                {studentName}
              </div>
              <div style={{ fontSize: 14, color: '#64748B', marginTop: 12, lineHeight: 1.7 }}>
                has successfully completed the full <strong>A1.1 English Elementary Level</strong> programme<br />
            ...
            <div style={{ marginBottom: 36 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                {[
                  { icon: '📖', label: 'Reading', desc: 'A1.1 texts & comprehension' },
                  { icon: '🎧', label: 'Listening', desc: 'Conversations & interviews' },
                  { icon: '✍️', label: 'Writing', desc: 'Reviews, stories, essays' },
                  { icon: '📐', label: 'Grammar', desc: 'A1.1 structures (M1–M17)' },
                  { icon: '📚', label: 'Vocabulary', desc: '900+ words across 18 modules' },
                  { icon: '🧩', label: 'Use of English', desc: 'Conditionals, passive, PP' },
                ].map(s => (
            ...
            {/* CEFR level band */}
            <div style={{
              background: `linear-gradient(135deg, rgba(22,163,74,0.04), rgba(201,147,59,0.06))`,
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  {['A1', 'A1.1', 'A2', 'B1', 'B2', 'C1'].map(lv => (
                    <div key={lv} style={{
                      padding: '4px 10px', borderRadius: 99, fontSize: 11, fontWeight: 800,
                      background: lv === 'A1.1' ? GR : '#F1F5F9',
                      color: lv === 'A1.1' ? '#fff' : '#CBD5E1',
                    }}>{lv}</div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 20, borderTop: `1px solid #F1F5F9` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  border: `2px solid ${GR}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column',
                  background: `rgba(22,163,74,0.05)`,
                }}>
                  <div style={{ fontSize: 7, fontWeight: 900, color: GR, textAlign: 'center', lineHeight: 1.2, letterSpacing: '0.05em' }}>KHAMADI{'\n'}ENGLISH</div>
                </div>
                <div>
