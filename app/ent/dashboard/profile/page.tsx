...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (!file.type.startsWith('image/')) { alert('Тек сурет файлдары рұқсат етілген (JPG, PNG, WebP)'); return }
      if (file.size > 5 * 1024 * 1024) { alert('Файл өлшемі 5 МБ-тан аспауы керек'); return }
