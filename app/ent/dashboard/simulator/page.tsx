...
export default function SimulatorPage() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (!currentUser) { setStarting(false); return }

      const { data, error } = await supabase
        .from('simulator_sessions')
        .insert({ user_id: currentUser.id, variant_id: 1 })
        .select('id').single()

      if (error) { console.error(error); setStarting(false); return }
      router.push(`/ent/dashboard/simulator/${data.id}`)
  ...
  return (
    <div>
      ...
      {/* Main hero card */}
      <div className="sim-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 20, marginBottom: 20 }}>
        {/* Left */}
        <motion.div
          {...fadeUp(0.08)}
          style={{
