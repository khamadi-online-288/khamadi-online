...
export default function StudyPlanPage() {
  ...
  async function toggleStatus(id: number, current: string) {
    ...
    if (next === 'done' && user) {
      await fetch('/ent/api/update-gamification', {
        method: 'POST',
  ...
  function getQuizLink(item: PlanItem) {
    ...
    return `/ent/dashboard/subjects/${item.subject_id}/sections/${item.section_id}?topic=${item.topic_id || ''}`
  }
      ...
      <div style={s.wrap}>
        ...
        {plans.length === 0 ? (
          <div style={s.emptyCard}>
            ...
            <a href="/ent/dashboard/ai-analysis" style={s.primaryLink}>
              AI Analysis-қа өту
            </a>
          </div>
        ) : (
